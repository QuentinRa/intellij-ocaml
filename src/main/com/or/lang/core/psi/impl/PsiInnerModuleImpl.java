...
public class PsiInnerModuleImpl extends PsiTokenStub<PsiModule, PsiModuleStub> implements PsiInnerModule {
    // region Constructors
    public PsiInnerModuleImpl(@NotNull ASTNode node) {
        super(node);
    }

    public PsiInnerModuleImpl(@NotNull PsiModuleStub stub, @NotNull IStubElementType nodeType) {
        super(stub, nodeType);
    }
    // endregion
    ...
    public boolean isInterface() {
        ...
        return psiElement != null && psiElement.getNode().getElementType() == OCamlTypes.TYPE;
    }
    ...
    @NotNull private Collection<PsiModule> getModules() {
    ...
    public @NotNull Collection<PsiNamedElement> getExpressions(@NotNull ExpressionScope eScope, @Nullable ExpressionFilter filter) {
        ...
        if (alias != null) {
            ...
            Set<PsiModule> modulesByName = psiFinder.findModulesbyName(alias, interfaceOrImplementation);
    ...
    private boolean isModuleTypeOf() {
        ...
        return nextSibling != null
                ...
                && nextSibling.getNode().getElementType() == OCamlTypes.TYPE
                && nextNextSibling.getNode().getElementType() == OCamlTypes.OF;
    }

    private @Nullable PsiModule findReferencedModuleTypeOf() {
        PsiElement of = ORUtil.findImmediateFirstChildOfType(this, OCamlTypes.OF);

        if (of != null) {
            // find latest module name
            PsiElement module = ORUtil.nextSiblingWithTokenType(of, OCamlTypes.C_UPPER_SYMBOL);
            PsiElement moduleNextSibling = module == null ? null : module.getNextSibling();
            while (moduleNextSibling != null
                    && moduleNextSibling.getNode().getElementType() == OCamlTypes.DOT) {
                PsiElement element = moduleNextSibling.getNextSibling();
                if (element != null && element.getNode().getElementType() == OCamlTypes.C_UPPER_SYMBOL) {
                    module = element;
    ...
    public ItemPresentation getPresentation() {
        ...
        return new ItemPresentation() {
            ...
            public @Nullable String getPresentableText() {
                if (isModuleTypeOf) {
                    if (referencedModuleType == null) {
                        PsiElement of =
                                ORUtil.findImmediateFirstChildOfType(PsiInnerModuleImpl.this, OCamlTypes.OF);
                        assert of != null;
            ...
            public @NotNull Icon getIcon(boolean unused) {
                return isInterface()
                        ? OCamlIcons.Nodes.MODULE_TYPE
                        : (isInterface() ? OCamlIcons.Nodes.INNER_MODULE_INTF : OCamlIcons.Nodes.INNER_MODULE);
            }
        };
    }
    ...
    public boolean isFunctorCall() {
        ...
        return ORUtil.findImmediateFirstChildOfType(this, OCamlTypes.C_FUNCTOR_CALL) != null;
    }
    ...
    public @Nullable String getAlias() {
        ...
        if (eq != null) {
            return ORUtil.computeAlias(eq.getNextSibling(), false);
        }
